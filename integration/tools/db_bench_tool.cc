...
DEFINE_string(
    ...
    "merge\n"
    //////////////////////////////////////////////////////////// Bulk Interface
    "\tseekscanrandom-- N random SeekScans, call NextScan seek_nexts times "
    "per seek\n"
    //////////////////////////////////////////////////////////// Bulk Interface
...
DEFINE_int32(seek_nexts, 0,
             ...
             "seekrandomwhilemerging, seekscanrandom");
////////////////////////////////////////////////// Bulk Interface
...
//DEFINE_bool(use_ce_rollback, false,
//            "If true, enable CE rollback in FlexDB.");
...
class Stats {
 ...
 public:
  ...
  void FinishedOps(DBWithColumnFamilies* db_with_cfh, DB* db, int64_t num_ops,
    ...
    uint64_t hard_pending_compaction_bytes_limit = 137438953472;
    /* --------------------------------------------------------- */
    if (reporter_agent_) {
    ...
    if (done_ >= next_report_ && FLAGS_progress_reports) {
      if (!FLAGS_stats_interval) {
        ...
        if (FLAGS_stats_interval_seconds &&
          ...
          /* [KI] Monitoring LSM STATUS */
          // 1. L0 files
          // 2. memtable size
          // 3. pending compaction size

          /* 1. number of files at level0 */          
          db->GetProperty("rocksdb.num-files-at-level0",&l0_num_str);
          l0_num_tmp = stoi(l0_num_str);
          /* 2. size of memtable */
          db->GetProperty("rocksdb.size-all-mem-tables", &memtable_size_str);
          memtable_size = stoull(memtable_size_str);
          /* 3. Pending compaction size */
          db->GetProperty("rocksdb.estimate-pending-compaction-bytes", &pending_size_str);
          //uint64_t hard_pending_compaction_bytes_limit = Options.hard_pending_compaction_bytes_limit;
          pending_compaction_bytes = stoull(pending_size_str);
          //set_memtable_size: 67108864
          //fprintf(stderr,"||%d||%ld||\n", l0_num_tmp, pending_compaction_bytes);
          //fprintf(stderr,"[Level Info] %d\n", l0_num_tmp);
          if(l0_num_tmp >= 36 || l0_num_tmp + (int)(memtable_size/67108864) >= 36 ||
            pending_compaction_bytes >= (hard_pending_compaction_bytes_limit - RESERVE)){
              fprintf(stderr,"[Monitor] STATUS H\n");
              //fprintf(stderr,"||%d||%ld||%d||\n", l0_num_tmp, pending_compaction_bytes, (int)(memtable_size/67108864));
            }
  ...
  void OpenDb(Options options, const std::string& db_name,
    ...
    if (FLAGS_num_column_families > 1) {
      ...
      if (s.ok() && FLAGS_secondary_update_interval > 0) {
        ...
        //options_for_bg.write_buffer_size = 536870912;
        ...
        std::cout << "DB_MASTER Open" << std::endl;
        s = db_master.DB_MASTER::Open(options_for_vlsm, db_name + "/vlsm", FLAGS_num_vlsm);
...
void SeekRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (FLAGS_use_tailing_iterator) {
        ...
        if (db_.db != nullptr) {
          db_master.IteratorReset(single_iter_master, options);
        } else {
      ...
      db_master.IteratorSeek(iter_to_use_master, key);
      read++;
      //if (iter_to_use->Valid() && iter_to_use->key().compare(key) == 0) {
      //  found++;
      //}
      //db_master.IteratorValid(iter_to_use_master) && 
      if (db_master.IteratorCompare(iter_to_use_master, key) == true){
        found++;
      }
      ...
      for (int j = 0; j < FLAGS_seek_nexts ; ++j) { //&& db_master.IteratorValid(iter_to_use_master)
       ...
       //printf("seek next\n");
        //Slice value = iter_to_use->value();
        Slice value = db_master.IteratorValue(iter_to_use_master);
        memcpy(value_buffer, value.data(),
        std::min(value.size(), sizeof(value_buffer)));
        //bytes += iter_to_use->key().size() + iter_to_use->value().size();
        bytes += db_master.IteratorKey(iter_to_use_master).size() + value.size();
        
        if (!FLAGS_reverse_iterator) {
          //iter_to_use_0->Next();
          db_master.IteratorNext(iter_to_use_master);
        } else {
          //iter_to_use_0->Prev();
  ...
  void ReadRandomWriteRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (get_weight > 0) {
        ...
        //printf("[DEBUG] ReadRandomWriteRandom_Get1\n");
        ...
        if (user_timestamp_size_ > 0) {
          Slice ts = mock_app_clock_->Allocate(ts_guard.get());
          //printf("[DEBUG] ReadRandomWriteRandom_Put1\n");
          s = db->Put(write_options_, key, ts, gen.Generate());
        } else {
          //printf("[DEBUG] ReadRandomWriteRandom_Put2\n");
